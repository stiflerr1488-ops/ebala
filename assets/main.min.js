...
// --- Tracking hooks (data-track) ---
      ...
      // --- SPA hook: re-bind page-level interactions after <main> swap ---
      function __npPageInit() {
        // update theme-dependent images inside swapped content
        try {
          const activeTheme = root.getAttribute("data-theme") || getSavedTheme() || getSystemTheme();
          updateThemeArt(activeTheme);
        } catch {}

        // re-run reveal animations + card interactions for new DOM
        try { initRevealAnimations(); } catch {}
        try { setupSpotlight(); } catch {}
        try { setupMagnetic(); } catch {}

        // re-bind anchors nav flash (hash links inside menus)
        try {
          const navLinks = Array.from(document.querySelectorAll('.nav a[href^="#"]'));
          navLinks.forEach((link) => {
            if (link.dataset.navHashBound === "true") return;
            link.dataset.navHashBound = "true";
            link.addEventListener("click", (event) => {
              const href = event.currentTarget.getAttribute("href");
              if (!href || !href.startsWith("#")) return;
              const target = document.querySelector(href);
        ...
        // re-bind copy-to-clipboard buttons
        try {
          const copyTargets = Array.from(document.querySelectorAll("[data-copy]"));
          if (copyTargets.length && navigator.clipboard) {
            copyTargets.forEach((target) => {
              if (target.dataset.copyBound === "true") return;
        ...
        // page forms / prefill
        try { initTelegramPrefill(); } catch {}

        // hash-driven UI
        try { handleHashChange(); } catch {}
      }

      try {
        window.NPMaps = window.NPMaps || {};
        window.NPMaps.pageInit = __npPageInit;
      } catch {}

      document.addEventListener("np:page", __npPageInit);
